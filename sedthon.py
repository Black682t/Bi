...
async def join_channel():
    try:
        await sedthon(JoinChannelRequest("@Source_Black1"))
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('أهلاً بك مطوري - @q_3_i')
...
async def update(event):
    await event.edit("• جارِ اعادة تشغيل سورس بلاك ..\n• انتضر 1-2 دقيقة  .")
